...
void event_loop_run(struct neowall_state *state) {
    ...
    fds[3].events = POLLIN;
    fds[4].fd = state->ipc_server ? ipc_server_get_fd(state->ipc_server) : -1;
    fds[4].events = POLLIN;
    ...
    while (atomic_load_explicit(&state->running, memory_order_acquire)) {
        ...
        if (ret == 0) {
            ...
            /* Handle IPC server events (client commands) */
            if (fds[4].revents & POLLIN) {
                if (state->ipc_server) {
                    ipc_server_process(state->ipc_server);
                }
            }

            /* Handle wakeup events (config reload, etc) */
        ...
        bool reload_flag = false /* reload removed */;
