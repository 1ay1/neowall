...
struct output_state *output_create(struct neowall_state *state,
    ...
    /* Initialize per-output cycling control */
    out->cycle_paused = false;
    ...
    if (output->config.cycle && output->config.cycle_count > 0) {
        ...
        while (sync_output && !index_set) {
            ...
            for (int check_slot = 0; check_slot < 2 && !index_set; check_slot++) {
                struct wallpaper_config *check_config = &sync_output->config;
                ...
                if (check_config->cycle &&
                    check_config->cycle_count == output->config.cycle_count &&
                    ...
                    if (same_config) {
                        ...
                        output->config.current_cycle_index = check_config->current_cycle_index;
