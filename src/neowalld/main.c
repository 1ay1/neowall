...
/* IPC command handler wrapper to match expected signature */
static void ipc_command_handler_wrapper(const ipc_request_t *req, ipc_response_t *resp, void *user_data) {
...
static bool write_ready_marker(void) {
    ...
    if (!fp) {
        log_error("Failed to create ready marker %s: %s", ready_path, strerror(errno));
        return false;
    }
    ...
    log_debug("Created ready marker: %s", ready_path);
    return true;
}

/* Remove ready marker file */
static void remove_ready_marker(void) {
    const char *ready_path = get_ready_marker_path();
    if (unlink(ready_path) == 0) {
        log_debug("Removed ready marker: %s", ready_path);
    }
}
...
void handle_signal_from_fd(struct neowall_state *state, int signum) {
    switch (signum) {
        ...
        case SIGHUP:
            ...
            // Hot reload removed - use restart instead
            break;
...
int main(int argc, char *argv[]) {
    ...
    const char *socket_path = NULL;  /* NULL = use default path */
    state.ipc_server = ipc_server_create(socket_path, ipc_command_handler_wrapper, &state);
    if (!state.ipc_server) {
    ...
    if (!wayland_init(&state)) {
        ...
        ipc_server_destroy(state.ipc_server);
        commands_shutdown();
        close(state.signal_fd);
        remove_pid_file();
        return EXIT_FAILURE;
    ...
    /* Write ready marker to signal successful initialization */
    ...
    /* Cleanup IPC server and commands system */
    if (state.ipc_server) {
        ipc_server_destroy(state.ipc_server);
    ...
    /* Remove ready marker and PID file */
    remove_ready_marker();
    remove_pid_file();
