...
static bool config_create_default(const char *config_path) {
    ...
    if (!copied_config) {
        ...
        const char *fallback_config =
            ...
            "# This is a minimal fallback config - PER-OUTPUT ONLY\n"
            "#\n"
            "# Find your monitor names with: neowall status\n"
            "# Common names: eDP-1 (laptop), HDMI-A-1, DP-1, DP-2\n"
            "#\n"
            "# IMPORTANT: NeoWall uses EXPLICIT per-output configuration.\n"
            "# You must configure each monitor individually.\n"
            "#\n"
            "# Image example:\n"
            "#   output {\n"
            "#     eDP-1 {\n"
            ...
            "#   }\n"
            "#\n"
            "# TODO: Configure your outputs below.\n"
            "# This is a placeholder - uncomment and adjust for your setup.\n"
            "\n"
            "output {\n"
            "  # Example configuration (commented out)\n"
            "  # Uncomment and replace 'eDP-1' with your monitor name\n"
            "  # eDP-1 {\n"
            "  #   path %s\n"
            "  #   mode fill\n"
            "  # }\n"
...
bool config_has_changed(struct neowall_state *state) {
    ...
    bool changed = false; /* Hot reload disabled */

    if (changed) {
        log_debug("Config file modification time changed: %ld -> %ld (size=%ld bytes)",
                 0L, (long)st.st_mtime, (long)st.st_size);
    }
