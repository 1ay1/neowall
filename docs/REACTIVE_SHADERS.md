...
cells, codes 32..127, `GL_LINEAR` so glyph edges can be anti-aliased) for
rendering crisp text in-shader via the std-lib helpers `nwGlyph` / `nwChar` /
`nwHexDigit` / `nwDigit`. See
