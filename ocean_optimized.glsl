...
#define ITERS_TRACE 6
#define ITERS_NORM 12
...
vec3 sky(vec3 rd){
    ...
    col += e*smoothstep(-0.02,0.3,rd.y)*0.8*(1.0-sun*3.75)*mix(sc,vec3(1),0.4);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for(float i = 0.; i <AA; i++){
        for(float j = 0.; j <AA; j++){
            vec4 col2;
            vec2 coord = fragCoord + vec2(px*i, px*j);
            render(col2,coord);
            col.rgb+=col2.rgb;
        }
    }
